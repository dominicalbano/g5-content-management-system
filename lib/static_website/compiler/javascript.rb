...
module StaticWebsite
  module Compiler
    class Javascript
      ...
      def compile
        LOGGERS.each{|logger| logger.info("about to call compile_directory.compile")}
        compile_directory.compile
        LOGGERS.each{|logger| logger.info("done calling compile_directory.compile")}
        LOGGERS.each{|logger| logger.info("about to call remote_javascript.compile")}
        remote_javascript.compile
        LOGGERS.each{|logger| logger.info("done calling remote_javascript.compile")}
