      ...
      context "valid instructions - single content stripe with widgets" do
        context "has drop target" do
          ...
          it "creates correct nested widget settings for content stripe widget" do
            expect(@response.child_widgets.size).to eq(cs_instructions[:widgets].size)
            expect(@response.child_widgets.first.slug).to eq(cs_instructions[:widgets].first[:slug])
            expect(@response.child_widgets.second.slug).to eq(cs_instructions[:widgets].second[:slug])
      ...
      context "valid instructions - content stripe with nested columns with widgets" do
        ...
        context "has drop target" do
          ...
          it "creates correct nested column widget settings for content stripe widget" do
            expect(@response.child_widgets.size).to eq(complex_instructions[:widgets].size)
          ...
          it "creates correct nested widget settings for nested column widgets inside content stripe widget" do
            expect(@first_col.child_widgets.size).to eq(complex_instructions[:widgets].first[:widgets].size)
            expect(@second_col.child_widgets.size).to eq(complex_instructions[:widgets].second[:widgets].size)
            expect(@first_col.child_widgets.first.slug).to eq(complex_instructions[:widgets].first[:widgets].first[:slug])
            expect(@first_col.child_widgets.second.slug).to eq(complex_instructions[:widgets].first[:widgets].second[:slug])
            expect(@second_col.child_widgets.first.slug).to eq(complex_instructions[:widgets].second[:widgets].first[:slug])
          end
        end
      end
    end
      ...
      context "valid instructions - single column with widgets" do
        context "has drop target" do
          ...
          it "creates correct nested widget settings for column widget" do
            expect(@response.child_widgets.size).to eq(col_instructions[:widgets].size)
            expect(@response.child_widgets.first.slug).to eq(col_instructions[:widgets].first[:slug])
            expect(@response.child_widgets.second.slug).to eq(col_instructions[:widgets].second[:slug])
